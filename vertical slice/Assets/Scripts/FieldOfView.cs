...
public class FieldOfView : MonoBehaviour
{
    ...
    ViewCastInfo ViewCast(float globalAngle)
    {
        ...
        RaycastHit hit;
        Debug.DrawRay(transform.position, dir, Color.red, viewRadius);
        if (Physics.Raycast(transform.position, dir, out hit, viewRadius, targetMask))
        {

            /* //GameObject.Find("weapon_cannon(Clone)").GetComponent<Turret>().isActivated = true;
             if(hit.collider.gameObject.GetComponent<Turret>().isActivated == false)
             hit.collider.gameObject.GetComponent<Turret>().isActivated = true;
